...
public class SwerveSubsystem extends SubsystemBase {
	...
	public SwerveSubsystem(VisionManager visionManager) {
		double subsysInitBegin = Timer.getFPGATimestamp();
		System.out.println("[INIT] SwerveSubsystem Init Begin");
		m_visionManager = visionManager;
		m_pigeon.getConfigurator().apply(new Pigeon2Configuration());
	...
	// public Command autoScore(Pose2d endPose) {
	 ...
	 */
	// public Command goToChosenPoint(Pose2d endPose) {
	// 	var follow = run(() -> {
	...
	// public Command autoAlign(Pose2d endPose) {
	// 	var follow = run(() -> {
	// 		addVision = false;
	// 		// double translationVal = MathUtil.applyDeadband(translation.getAsDouble(),
	// 		// Constants.stickDeadband);
	...
	// public Command goToConeOrCube(DoubleSupplier translation, boolean isCone) {
	...
	// protected Command ppFollowerCmd(PathPlannerTrajectory traj) {
	...
	// public Command getPPSwerveAutonCmd(PathPlannerTrajectory trajectory) {
	...
	// public Command getPPSwerveAutonCmd(List<PathPlannerTrajectory> trajList) {
	// 	if (trajList.size() == 0) {
	...
	// 		List<Command> pathCmds = new ArrayList<Command>();
	...
	// 		Command[] pathCmdsArr = pathCmds.toArray(new Command[0]);
	 ...
	 */
	// public Command rotateAboutPoint(double degrees) {
	// 	return run(() -> {
