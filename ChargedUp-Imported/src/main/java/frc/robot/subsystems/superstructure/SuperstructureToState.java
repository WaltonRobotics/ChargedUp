...
public class SuperstructureToState extends SequentialCommandGroup {
    ...
    public SuperstructureToState(Superstructure superstructure, SuperState targetState, boolean proxyInternal) {
        ...
        Command wristCmd = Commands.waitUntil(m_wristWait).andThen(wrist.toAngle(m_targetState.wrist.angle));
        Command elevCmd = Commands.waitUntil(m_elevWait).andThen(elevator.toHeight(m_targetState.elev.height));
        Command tiltCmd = Commands.waitUntil(m_tiltWait).andThen(tilt.toAngle(m_targetState.tilt.angle));
